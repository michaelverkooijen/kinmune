...
payload = {'action': 'query', 'list': 'embeddedin', 'eititle': 'Template:SkyrimGiantCamps', 'eilimit': '5000', 'format': 'json'}
...
for page in r.json()['query']['embeddedin']:
    ...
    body = body.replace('{{SkyrimGiantCamps', '{{SkyrimLocations')
    #TODO: test body size, break if over 8k.

    payload = {'action': 'edit', 'title': page['title'], 'summary': 'Infobox migration', 'bot': '1', 'watchlist': 'nochange', 'format': 'json', 'text': body, 'token': edit_token}
